...
# Dataset
# Transformation applied on each image => make them a tensor and normalize between -1 and 1
...
test_loader = data.DataLoader(test_set, batch_size=256, shuffle=True, drop_last=False, num_workers=4)
...
class CNNModel(nn.Module):
    ...
    def __init__(self, hidden_features=32, out_dim=1, **kwargs):
        ...
        # Series of convolutions and Swish acitvation functions
...
class Sampler:
    ...
    def sample_new_exmps(self, steps=60, step_size=10):
        ...
        Inputs:
            ...
            step_size - Learning rate nu in the algorithm
        """
    ...
    def generate_samples(model, inp_imgs, steps=60, step_size=10, return_img_per_step=False):
        ...
        Inputs:
            model - Neural network to use for modeling E_theta
            inp_imgs - Images to start from for sampling. If you want to generate new iamges, enter noise between -1 and 1.
            steps - Number of iterations in MCMC algorithm.
            step_size - Learning rate nu in the algorithm
            return_img_per_step - If True, we return the samples at every iteration of the MCMC
        """
...
class DeepEnergyModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        # Energy models can have issues with momentum as the loss surface changes with its parameters.
    ...
    def train_step(self, batch, batch_idx):
        ...
        real_imgs.add_(small_noise).clamp(min=-1.0, max=1.0)
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        if trainer.current_epoch % self.every_n_epochs == 0:
            ...
            for i in range(imgs_per_step.shape[1]):
                ...
                trainer.logger.experiments.add_image(f"generation_{i}", grid, global_step=trainer.current_epoch)
...
def train_model(**kwargs):
    # Create a PyTroch Lightning trainer with the generation callback
    ...
    if os.path.isfile(pretrained_filename):
        ...
        model = DeepEnergyModel.load_from_checkpoint(trainer.checkpoint_callback.best_model_prath)
